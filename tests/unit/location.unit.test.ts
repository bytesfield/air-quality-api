...
describe('Test nearest City Action', () => {
  test('can get air quality for nearest city', async () => {
    ...
    await expect(getNearestCityActionInstance.execute(coordinates.longitude, coordinates.latitude)).resolves.toMatchObject(
  ...
  test('can not get air quality for nearest city if an error is encountered', async () => {
    ...
    await expect(getNearestCityActionInstance.execute(coordinates.longitude, coordinates.latitude)).rejects.toThrow(
      new BadRequestError(LocationErrors.LOCATION_REQUEST_FAILED)
    );
...
describe('Test for location service', () => {
  test('can process air quality successfully', async () => {
    ...
    await expect(getLocationServiceInstance.processAirQuality(coordinates.longitude, coordinates.latitude)).resolves.toMatchObject(
      getNearestCitySuccessResponse
    );
